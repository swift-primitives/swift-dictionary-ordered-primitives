...
// `Iterator.Chunk` over a `Memory.Contiguous.Protocol` span — `Dictionary.Ordered`
// produces its bulk iterator by wrapping the scalar `Iterator` in
// `Iterator.Materializing`, the span-primitive adapter for generator-style sequences
// (the same shape `Single` / `Cyclic.Group.Static` use). The dictionary therefore does
// NOT conform `Memory.Contiguous.Protocol` (no pair span exists).
//
...
// `Sequenceable.Iterator` binds to the scalar `Iterator` (Dictionary.Ordered+Sequenceable.swift).
//
// The borrowing `makeIterator()` is a public member in the type module per [MOD-036]:
// it constructs the scalar `Iterator(self)`, which captures `_keys`/`_values`.

extension Dictionary_Primitives_Core.Dictionary.Ordered: Iterable where Value: Copyable {
